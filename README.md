...
## Java Plus Graduation — microservices
...
Учебный проект построен по принципу микросервисной архитектуры, где каждая бизнес-функциональность вынесена в самостоятельный сервис с конкретной задачей, обеспечивая высокую масштабируемость и надежность системы.
...
- **Модуль stats**
    - **Stats Server** (`stats-server`) — статистика
- **Хранилища**
    - `event-db`, `request-db`, `user-db`, `stats-db`
